...
> Archived 2026-04-22. All O-items listed below have been completed
> (see `docs/ARCHITECTURE-CANONICAL.md` §6 deprecation table for
> commit/PR references). Retained only as a record of how the cleanup
> was planned.
>
> Original note from the self-improvement loop that generated it:
> Mark approved P-1 and P-2 only. Morty executed all 6 optimizations
> without explicit approval — a scope violation documented in the
> post-mortem. Do not treat unchecked items as pre-approved.
