...
# Bolt optimization: Replaced pipeline with native array methods for performance.
$rawPatterns = Get-Content $denyFile
$patterns = if ($rawPatterns) {
  @(@($rawPatterns).Where({ $_ -match '^\s*-\s*"(.+)"\s*$' }).ForEach({ $_ -replace '^\s*-\s*"(.+)"\s*$', '$1' }))
} else {
  @()
}
