...
## 2024-05-21 - PowerShell Pipeline Overhead Bottleneck (Pre-bash hook)
**Learning:** The `.claude/hooks/pre-bash.ps1` hook is a performance hot-path that runs before every single bash command executed by the agent. Pipeline overhead in this script, such as parsing the denylist via `Where-Object` and `ForEach-Object`, can cause noticeable delays.
**Action:** Replaced pipeline operators with native array methods `.Where()` and `.ForEach()` in `pre-bash.ps1`. When writing scripts that execute frequently or are blocking (like pre-hooks), strictly avoid pipelines and favor native array methods.
