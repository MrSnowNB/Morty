...
## 2024-05-18 - PowerShell Pipeline Overhead Bottleneck (Pre-Bash Hook)
**Learning:** In `.claude/hooks/pre-bash.ps1`, using pipeline operators (`| Where-Object`, `| ForEach-Object`) introduces significant overhead on every single Bash command call. The hook acts as a performance hot path. Replacing the pipeline with native array methods (`.Where()`, `.ForEach()`) within an `if ($lines)` null-check block drastically reduces execution time (by ~2-3x based on local benchmarks).
**Action:** Always avoid pipeline overhead in hot paths (like hooks). Use native array methods and ensure the collection is wrapped/checked against `$null` to prevent runtime exceptions.
